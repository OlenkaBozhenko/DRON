...
| **Nav-bar back button** | Apple HIG: the chevron + the **title of the screen you return to** — `‹ {previous screen}`; never `Back to …` (the chevron already means "back"). Bare `‹ Back` only when the previous screen has no title. A **tab-root** screen has no back button (you reach it by switching tabs). *(In-content recovery buttons in empty/error blocks are a different element — content CTAs like `Back to Jobs` — and stay descriptive.)* | `‹ Order review` (on Payment) · `‹ Sign in` (on Verification) — not `‹ Back to Order review`, not a bare `‹ Back` | HIG (mobile client) · P4 · `microcopy.md` D1 |
