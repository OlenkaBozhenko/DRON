...
| order-details | Payment | Paid | Field label | **Paid**, not *Total*. `voice.md` fixes *paid* as money that has landed; `order-review`'s `Total` is what you are about to be charged, and the two screens must not use one word for both. **THE ZONE HOLDS THIS ONE STRING AND NO BREAKDOWN** — the designer, 2026-08-16 (rev 132): *«на цих wireframes що показують деталі вже виконаного замовлення лише остаточна сума без розшифровки»*. `Base fare` / `Distance · 7.4 km` / `Service fee` were built into the HTML at rev 130 and were never listed here; they belong to `order-review`, where **P3** puts the price before the tap. A record states what was charged, not how it was computed |
...
| order-details-inspection | Summary | Location · Structure · Delivered · Operator | Field labels | **Location**, not *From ▸ To* — an inspection has one stop, and the card it opens from already shows a single `Location` row. **Structure** is the inspection's analogue of `Parcel`: the thing the order was about. **Delivered** is the same word the delivery record uses, and for the same reason — `voice.md` keeps one word per concept, and the concept is *the deliverable landed*, not *a parcel landed*; all three cards already carry `Delivered` as the accessible name of their settled check |
...
| order-details-inspection | Payment | Paid | Field label | the one string, no breakdown — rev 132's rule holds for every record file |
| order-details-inspection | Payment | ₴650 | Values | DATA — the sum on the card that opens it |
...
| order-details-aerial | Summary | Location · Shoot · Delivered · Operator | Field labels | **Shoot** is this service's analogue of `Parcel` / `Structure` — the thing the order was about. It is the client's own word for a booked photo session, not equipment (`voice.md` **P4**); *Job* was not available for it, since `voice.md` reserves *job* for the operator side |
...
| order-details-aerial | Photo & video set | Six people in helmets and life vests riding a raft over a drop in white water, shot from above. | Alt text | describes the frame and only the frame. It does **not** repeat *Trukhaniv Island* from the row above: with the label kept against the picture (see the Values row), an alt that echoed the label would state as fact something the image contradicts, which is the failure `WCAG 1.1.1` exists to prevent |
| order-details-aerial | Payment | Paid | Field label | as the other two records |
| order-details-aerial | Payment | ₴800 | Values | DATA — the sum on the card, and the same ₴800 `wallet` shows landing on the operator's balance for this service |
