...
| tracking · -empty · -error | Contact drawer | Send message | Button | `voice.md` **A9** verbatim — *"to a person: **Send** (Send invite, Send message)"*. Deliberately **not `Chat`**, which is a noun and names the room instead of the act (the Button rule bars a bare noun), and deliberately not *Message operator*, which would put the object in all three rows and make the drawer title redundant twice over. The file it opens is `chat.html`, because *chat* is the object's name in `sitemap.md §4` — a file name is not a product string |
