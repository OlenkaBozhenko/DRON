...
| order-details-aerial | Photo & video set | Download | Button | **the designer's own verb** — *«можливістю скачати відео»*, `скачати` = download — fixed once in `voice.md` as **A11**: **Download**, never *Save*, *Export* or *Get*. It is not `A3 Add` reversed (*Add* is the client putting a file **into** DRON: `Add photo`, `Add document`) and not `O6 Withdraw`, which is reserved for money leaving a balance. Visible label is the one word; the accessible name is `Download video`, which contains it — `WCAG 2.5.3 Label in Name` |
...
| order-review | Pay drawer | Total ₴180 — base fare ₴90, distance ₴70, service fee ₴20 | Body | rev 136 — the sheet's **message**, the optional line `HIG · Action sheets` puts under a title, tied to the dialog with `aria-describedby`. The designer's call, taken from three measured options: *«добав сюди якокось опису трохи тотал пеймент etc»*. **It is not a repeat of what the reader can see** — measured, the money card stands at y **395–592.4** and the sheet's top edge at **488.4**, so the drawer covers `Service fee ₴20` and `Total ₴180` and cuts `Distance` in half; `Base fare ₴90` is the only row still whole behind the scrim. While the sheet is up, this line and the title are the only place the total exists. It is therefore not the sentence retired from this screen on 2026-08-15 — *"This is the final price. Nothing is added at checkout."* was a **promise** about the price; this is the **price**. Sentence case, no full stop, the em dash the total already earns; the figures are `DATA` and change with the order |
