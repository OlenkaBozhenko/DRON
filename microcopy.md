...
| switch-role | What happens | Switching is always reversible from the same Account entry. | Body | the zone's only string since 2026-09-10 (rev 234), when the designer deleted the fact panel above it (*«delete this block for both roles operator and client»*). It is the one of the three facts that survives: *Switch back → Any time from Account* said the same thing in two cells |
