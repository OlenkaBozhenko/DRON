...
| job-checklist-error · job-checklist | Contact drawer | ~~Calls and messages go through DRON. Your number is not shared with the client.~~ | Body | **REMOVED from `job-checklist` 2026-08-24** on the designer's word (*«delete this sentence»*), and recorded rather than dropped quietly — the second string this table keeps as struck-through, after `call`'s own masking line at rev 144. What it did: `tracking`'s masking sentence **with the object turned round, and true in that direction too** — the masking is bilateral, so the operator has never been shown the client's number either — `voice.md` **P3**'s discipline, state the fact **before** the tap, applied to privacy instead of to price. **What its removal costs, stated:** with `call`'s copy already gone at rev 144, the operator side now states the masking fact **nowhere**; the client side still states it, in the four `tracking` drawers. The structure it explained is unchanged — *Call client* is still a row with no number on the frame, opening DRON's own call screen and not a `tel:`. `aria-describedby` came off the drawer with it, an IDREF resolving to nothing being invalid (ARIA 1.2); the drawer keeps its name from `aria-labelledby` (`WCAG 4.1.2` ✓). **Still live on `job-checklist-error`** — the error twin was not named in the instruction and is not touched without her word |
