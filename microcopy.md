...
| job-offer | Offer | 2.3 km · ~18 min | Body | DATA · **split off its second half 2026-09-03 (rev 220)** — the designer, against the built card: «pickup near Podil it is a second row». The string was one line that *wrapped* at a 193px measure, so the break fell where the measure put it; it is now the first of two `.dr-listing__desc` siblings. What the middle dot was joining is two facts that do not sit on the same footing — how far and how long is the trip, and where it starts |
| job-offer | Offer | Pickup near Podil | Body | DATA · **new row 2026-09-03 (rev 220)**, the second half of the line above. **Capitalised on the split, which is more than she asked for and is hers to reverse** — a row that starts its own line starts with a capital, and every other `__desc` in the product does; lowercase `pickup` was a mid-sentence fragment after a `·`. It restates the Route zone's **Pickup ▸ Podil, 21** in shorthand, one screen-height above it — an offer card exists so the operator can decide without scrolling, so the repetition is the point rather than a defect. `job-offer-inspection` carries the same split with **Obolon district** on its second row (that file is not inventoried here, the master table holding base screens only) |
