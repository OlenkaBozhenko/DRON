...
| account-edit | Notifications | Push works only after you allow it on this device. | Body | new 2026-08-24 — the card's one note. `HIG · Requesting permission` puts the ask in context, not in settings, so this screen states the dependency rather than pretending the switch is the whole story. P5 — plain and accountable: what happens, not an apology for it. **On this device** is the load-bearing half; MVP is mobile web, so the permission is per-browser |
