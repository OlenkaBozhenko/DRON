...
| call | Who | Andriy M. · 4.9 ★ · 214 jobs · Verified by DRON · Insured | Body | DATA — **the `.dr-op` card byte-identical to tracking's**, not a restatement. It is here instead of an iOS-style round portrait because `visuals/gaps.md` B1 #4 says the portrait does not exist: the one human cutout is shot from behind. `RJ-C1`'s two chips therefore stand ON the call screen rather than only behind it |
| call | Call | 02:14 | Body | DATA. **The running clock is the statement that the call is up**, so no second line says *Connected* — `voice.md`'s own rule, state the fact and cut what decorates it. 20/700 **tabular**, because a duration that reflows its own width every second is what tabular figures exist to stop |
...
| call | Call | The call goes through DRON. Your number is not shared with the operator. | Body | the drawer's masking fact, repeated **where it matters most** — the client is on the line with a stranger, which is the moment it is worth reading. Two facts, no adjective (`voice.md` **P2**). Not a duplicate of the drawer's line but a re-tense of it: *calls and messages go* before the tap, *the call goes* during it |
| call | Controls | Mute | Button | the toggle's one name in both states — it does **not** become *Unmute* when pressed, because `aria-pressed` carries the state and a label that flips makes the control announce two different things (`WCAG 2.5.3`, `4.1.2`). iOS labels its call controls the same way |
| call | Controls | Speaker | Button | same shape, same rule. Not *Speakerphone* — `voice.md` **P4** takes the word the client would say |
| call | Controls | End call | Button | verb + object, the Button rule. Not *Hang up* (a landline idiom C-1 does not use), not *End* alone (a bare verb with no object), and **not** *Cancel* — the call happened, it is being finished rather than undone. It is the screen's only exit, which is what `HIG · Modality` requires of a modal view |
