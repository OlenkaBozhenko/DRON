...
| switch-role | switch-role.html | What happens (fact panel) | body | In-progress order · Kept — resume from Client / First time as Operator · Runs onboarding once / Switch back · Any time from Account | **cut from the build 2026-09-10 (rev 234)** — the designer, against the built block: *«delete this block for both roles operator and client»*. One file serves both directions (`_screens.md` S1, base only), so one deletion covers Client→Operator and Operator→Client alike. **Six strings retire and one fact is kept, not deleted**: *Switch back → Any time from Account* is what the `.dr-note` under it already says in a sentence — *Switching is always reversible from the same Account entry.* — and that note stays. **Two facts leave the product entirely and are recorded here rather than lost quietly**: that an in-progress order survives the switch, and that the first switch runs that side's onboarding once. Both still live in `flows.md` and in the page's `.meta` panel; neither is said to the user any more. `voice.md` P5 is not engaged (no trouble state), P2 is: the panel was proof, and the screen now proves less. Open for her word. |
| switch-role | switch-role.html | What happens (note) | body | Switching is always reversible from the same Account entry. | **cut from the build 2026-09-10 (rev 237)** — the designer, against the note whose break she had set that same morning: *«Remove this and all the other content aligne by center of the page»*. **The words read as a deletion of the whole centred block, so she was asked before anything was touched** — the photo mark, the `Switch to Operator?` heading and the lead deleted with it, or kept and left-aligned, or the note alone. She chose **the note alone**: the `h1` and the lead stay exactly as built, centred, and only this string leaves. **The zone leaves with it** — `WHAT HAPPENS ▸ NOTHING IS LOST` had nothing left to head once rev 234 took the fact panel and this took the note, so `main` is one zone (`SWITCH`) and the action bar. **Third fact out of the product in one day**: that an in-progress order survives the switch, that the first switch runs onboarding once, and now that the switch is reversible at all. All three still live in `flows.md` and in the page's `.meta` panel; none of them is said to the user. `voice.md` P2 (*show the proof*) is what is being spent down — the confirm now proves only what the lead says. **The `<br>` departure of rev 236 is retired with the line it was written for** (`_conventions.md` §95, `DESIGN.md`), recorded so it is not re-proposed as a new idea. |
...
**Sync — 2026-08-16 (`delivery-aerial`'s confirmation line comes off, and the nav bar states the outcome alone)**
- **One string retired on the designer's word**, given on the built page — *remove this phrase*: **"Your photo & video set is ready."** Its master-table row is gone. It is the fourth sentence to come off a built screen this way, after `order-confirmed-empty`'s reassurance, `order-review`'s price line and `account-edit`'s phone hint.
- **Nothing arrives to replace it.** The `.dr-center` block keeps the milestone mark and loses its only text; `.dr-center`'s `gap` applies between children, so with one child left no spacing is stranded.
- **The outcome is still stated in text, once.** The nav bar's `<h1>` reads **`Done`** — the fourth rail node's own word on `tracking` — and the zone label below reads **`Photo & video set`**, so *what happened* and *what came back* are both named without the sentence. This is the same move `delivery` made on 2026-08-15 under *«не повинно залишитись ніде два заголовка»*, one line further: there the duplicate display heading went, here the sentence that paraphrased the bar goes too.
...
**Sync — 2026-08-31 (the alert log — 27 strings enter the table, and the product finally lets someone SEE what it has always sent)**

- **The designer's call, given against the built `listings` top bar:** *«Add on the right side of the header on the home page, in front of the DRON button icon, "notification" for both client and operator modes. And add wireframes by clicking this button with notification history.»* Two screens plus their states, and a control on each home bar.
- **This is the second half of the 2026-08-24 sync, not a new subject.** That pass gave the product a place to **set** notifications and wrote down, in `sitemap.md §7.4`, that the product had always **sent** them (`CE-4`'s push, `OE-6`'s dispatch) and never let anyone see them. An alert lands on a lock screen and is gone. This is where it is kept.
- **Twenty-seven strings, and three of them are deliberately NOT new.** *Browse services* is `order-history-empty`'s exit, *Find jobs* is `wallet-empty`'s, and *No notifications yet* is one title used on both empty screens. **A8**'s rule — one phrasing per request — is the reason: the same door in the same journey does not get a second name because a new screen wanted one.
- **The wall between the two vocabularies is the whole reason there are two screens.** The client's rows say **order**; the operator's say **job**; neither crosses. The money words split the same way and inside one screen: **paid** for earnings landing, **withdrawal** for money going out, one group apart. *Operator*, never *pilot* — and on the client's `Ivan is 2 minutes away` the operator has a **name**, because by the time that alert fires the client has already been shown his face.
- **`Notifications` is now two things and they are told apart in `sitemap.md`, not here.** The `§7.4` Deep row became **Notification settings**; the log keeps the plain noun. One concept, one word, and the disambiguation done where the navigation model lives rather than by decorating a string.
- **Two strings are accessible names and exist nowhere on screen.** *Notifications, 3 unread* on the bell (the pill is `aria-hidden`, so the count would otherwise reach only a sighted reader — `WCAG 4.1.2`) and *Back to Home* / *Back to Jobs* on the chevron (**D9**: forbidden as visible copy, required as a name). The bell's name takes a **comma**, not the middle dot the visible meta rows use — it is read aloud, not laid out.
- **`New` and `Earlier` are copy doing a job a colour would otherwise do.** They are the unread signal, which is why `WCAG 1.4.1 Use of Color` is not engaged on either screen. *Earlier*, not *Read*: one describes when it happened, the other describes the system's bookkeeping, and only the first is what a reader scans for.
- **No toast anywhere.** Tapping a row navigates, and this file's own rule already covers it: a screen change is visible either way.

**Sync — 2026-08-24 (`account-edit` gets a Notifications card — eight strings enter the table, and the product finally lets someone set what it has always sent)**
- **The designer's two calls**, taken one at a time on a choice that was put to her with the standards first: **«third card, inline»** (a card in place, not a row that pushes its own screen), then **«make a toggle»** (a switch, not the kit's checkbox).
- **Eight strings are written, and that is eight more than any recent sync** — this is a new card, not a rewrite, so nothing retires. Two are the card's title and subtitle, five are toggle labels, one is the note. **`Toggle label` is a new Type in this table**, and it is a real distinction rather than a synonym for *Field label*: a field label names a value beside it, a toggle label **is** the row and is the control's whole accessible name.
- **The vocabulary was checked against `voice.md` before any of it was written, not after.** *Order updates* takes **order**, the client-side word, never *job*. *Operator on the way* takes **operator**, never *pilot*, and it names the outcome rather than the equipment (**P1**) — it is `CE-4`'s own push, the *"Operator is 2 min away"* the sitemap has quoted since the first cut. *Offers and news* is deliberately flat: `voice.md` bars the cheery register, so no *Deals*, no *perks*, no exclamation.
- **The note is the one string doing real work, and `HIG · Requesting permission` is why it exists.** *Push works only after you allow it on this device.* Settings is where a permission is **changed**; the first ask belongs in the flow, in context, with its reason stated. Rather than let a switch imply it can grant what only the OS can, the card states the dependency. **P5** — plain and accountable, no apology, no *unfortunately*. **On this device** is the load-bearing half: MVP is mobile web, so the permission is per-browser, and on iOS Safari it needs Add-to-Home-Screen at all (`sitemap.md §7.4` `[?]`). — **cut from the build 2026-08-31 (rev 210):** the designer took the sentence off the card (*«remove this sentence»*). The reasoning stands as history; the string no longer ships, so the card is now five switches and nothing under them, and no line in the product says the OS holds the last word on Push. The `[?]` in `sitemap.md §7.4` is untouched — where the first ask lives was never this note's job.
- **One string is deliberately NOT written, and it is the operator's.** The screen is shared — `account.html` and `operator-account.html` both point *Edit profile* here — and `OE-6`, the dispatch alert, gets no row: it is the mechanism `RJ-O1` runs on, and `OE-4` (Available / Busy / Offline) is already the operator's control for *not now*. A switch offering to silence dispatch would be copy that promises something the product must not do.
- **No email row, for the same reason.** There is no email anywhere in `CE-1` — registration is Diia / BankID and the account holds a phone — so *Push* and *SMS* are the two channels that exist. A third would be inventing an entity in a string.
- **The no-toast verdict holds without a new argument.** Every row here is a switch, and this file's own rule already covers it: *move a selection — visible either way*.

---

**Sync — 2026-09-03 (both alert logs stop being grouped by read-state and start being grouped by day — two headings retire, four strings and one format arrive)**

- **The designer's call, given against the built `operator-notifications` and anchored on the `New` heading:** *«write today, yestaday and date if it more than yesterday and fix the mergings and puddongs it should be viseal notised differen dates etc»*. Two asks in one line: the **words** that name a group, and the **space** that separates one group from the next.
- **THE STANDARDS WENT FIRST AND ONE OF THEM WAS ALREADY FAILING.** `HIG · Lists and tables` gives a recent date **relatively** — *Today*, *Yesterday* — and an absolute date past that. The built log had **no *Yesterday* at all**: it jumped *Today* → *28 Jun*, so a two-day-old alert was formatted identically to a sixty-seven-day-old one. `WCAG 1.3.1` was satisfied before and after (each group is a `<section aria-labelledby>` with its own `<h2>`), and `WCAG 1.4.3` passes at **6.62:1** on the heading, unchanged.
- **`New` / `Earlier` retire, and what they were carrying is stated rather than dropped.** The 2026-08-31 sync recorded them as *copy doing a job a colour would otherwise do* — they **were** the unread signal, which is why `WCAG 1.4.1` was never engaged. Replacing them with dates would have left unread with nothing to sit on, so **the signal moved to the row** as a `New` chip. **It is still a word**, so `1.4.1` is still not engaged; the fill under it is decoration the word does not depend on.
- **The move is what the two axes were always asking for.** *New* and *Earlier* pretended read-state and recency were one axis. They are not, and the operator's log now proves it on its own face: **Your documents are approved** is unread and sits under **Yesterday**. Under the old headings that row had to lie about one of the two facts.
- **A8 is why only four strings are authored for two screens.** *Today* and *Yesterday* are inventoried once, under `notifications`, because they are the identical two words on both sides of the wall. The absolute headings are **a format, `d MMMM`, not a string** — day-first for UA and the EU, month spelled out because a heading is read once. The value is data; what this file owns is the shape.
- **Two more strings are rewritten, and both are the chip paying for its own space — the budget is written down so the next title can be written to it.** A title carrying a chip does not get the row's **309**: the pill and its gap take **48.28**, leaving **260.72** at the standard tier and **236.72** at the narrow one (≤ 389px, frame 351). Both tiers were measured, not one.
- **The second of them, and it is the one that cost a pronoun.** *New job offer · Package delivery* → **Job offer · Package delivery**. The row's chip already says *New*, so the title was restating it — and the chip plus its gap take **48.28** of the **309** text column, so a title that carries a mark is written to **260.72**. At the title size restored the same afternoon the old string crossed that line and wrapped, taking its row to **101.78** beside a neighbour's **77.98**. **Rephrased, never truncated** — the remedy the `support` pass took on the same component.
- **Two row-meta lines are rewritten and both are shorter, which is A8 again.** *Package delivery · Today, 10:12* → *Package delivery · 10:12*; *Podil → Osokorky · Today, 10:45* → *Podil → Osokorky · 10:45*. The day left the row because the heading above it now carries the day, and a fact stated twice in twenty pixels is a fact the reader stops trusting.
- **Sample dates moved on both logs, and that is said plainly rather than presented as discovery.** The client's *Rate your order* went 28 Jun → **Yesterday** and the operator's *Your documents are approved* went Today → **Yesterday**, because neither log held a yesterday and the pattern the designer asked for could not otherwise be seen. Demo data, changed on purpose, on a wireframe whose job is to show the pattern.
...
| account-edit | Notifications | Operator on the way | Toggle label | new 2026-08-24 — `CE-4`'s live push, the one the sitemap has quoted since the first cut (*"Operator is 2 min away"*), serving `RJ-C2`. **Operator**, never *pilot*. P1 — it names what happens to you, not the drone that does it |
| account-edit | Notifications | Offers and news | Toggle label | new 2026-08-24 — the marketing channel, and the one row that ships **off**. Plain and unsold: `voice.md` bars the cheery register, so it is not *Deals & perks* |
| account-edit | Notifications | Push | Toggle label | new 2026-08-24 — the channel. P4, the user's word: *Push* is what the OS prompt itself says, where *Push notifications* restates the card's own title |
| account-edit | Notifications | SMS | Toggle label | new 2026-08-24 — the other channel, and on iOS possibly the only reliable one (the `[?]` in `sitemap.md §7.4`). *SMS*, not *Text message*, is the Ukrainian user's word |
...
| account-photo | Photo | JPG or PNG, up to 5 MB. Nothing changes until you save. | Body | P3 — two facts before the tap: the file rule, and that **no button on this screen commits on its own**. The second sentence carries what `You can retake it before saving.` said under the removed upload card, and covers `Delete` as well as the two sources — which is what makes the destructive action recoverable without a confirmation alert. **IT MOVED OUT OF THE ACTION BAR ON 2026-08-27 (rev 201)**, the designer on the built frame: «move this sentences with requirments to the card with avatar under: Your operator sees this photo when they arrive» — so it is now the second line of the photo card, under the sentence that says why the picture matters, and the two ride `.dr-stack`'s 8 as one voice. It was `HIG · Action sheets`' optional message slot; that slot is now empty, which the guideline permits. `WCAG 3.3.2` still has its instruction and now **earlier** in reading order, and the three buttons keep `aria-describedby` — the description computes by **id**, so nothing they announce changed (`4.1.2`, `1.3.1`). **The operator twin took the same move an hour later** (rev 202), so the line now stands in the photo card on both screens — the sentence above it is the only thing that differs. **AND FROM 2026-08-27 IT IS TWO ROWS BY DECISION, NOT BY ACCIDENT** — *«make in 2 rows but it should be more text in second row apply this chsnges for 2 profile roles»*: the card's 301 measure had been breaking it `…Nothing changes until you` / `save.`, a one-word orphan, and `.dr-note--balance` (`text-wrap: balance`) puts the break on the **sentence boundary** instead — **`JPG or PNG, up to 5 MB.`** then **`Nothing changes until you save.`**, one fact per row and the second row 8 characters longer. **No word changed**, so this row's string is the string rev 128 wrote; a `<br>` was refused for the same reason `.dr-display--balance` refused one — the break belongs to the measure, not to the copy, and a translation re-balances itself |
...
| job-checklist-error · job-checklist | Contact drawer | ~~Calls and messages go through DRON. Your number is not shared with the client.~~ | Body | **REMOVED from both checklist states 2026-08-24** on the designer's word (*«delete this sentence»*), and recorded rather than dropped quietly — the second string this table keeps as struck-through, after `call`'s own masking line at rev 144. What it did: `tracking`'s masking sentence **with the object turned round, and true in that direction too** — the masking is bilateral, so the operator has never been shown the client's number either — `voice.md` **P3**'s discipline, state the fact **before** the tap, applied to privacy instead of to price. **What its removal costs, stated:** with `call`'s copy already gone at rev 144, the operator side now states the masking fact **nowhere**; the client side still states it, in the four `tracking` drawers. The structure it explained is unchanged — *Call client* is still a row with no number on the frame, opening DRON's own call screen and not a `tel:`. `aria-describedby` came off the drawer with it, an IDREF resolving to nothing being invalid (ARIA 1.2); the drawer keeps its name from `aria-labelledby` (`WCAG 4.1.2` ✓). **Both states, so the parity rule holds** — `concept.md` rev 26's *the same control at the same weight in every state*; the drawer block is verified byte-identical across the two files after the cut. The client's nine `tracking` drawers went in the same sweep, on her word (*«everywhere»*); see the `tracking` row |
...
| job-offer | Offer | 2.3 km · ~18 min | Body | DATA · **split off its second half 2026-09-03 (rev 220)** — the designer, against the built card: «pickup near Podil it is a second row». The string was one line that *wrapped* at a 193px measure, so the break fell where the measure put it; it is now the first of two `.dr-listing__desc` siblings. What the middle dot was joining is two facts that do not sit on the same footing — how far and how long is the trip, and where it starts. **The split is undone at rev 223** — the sibling row was deleted on the designer's word, so this string is the whole description again: one line of the 193px measure, and this time it does not wrap |
| job-offer | Offer | ~~Pickup near Podil~~ | Body | **CUT 2026-09-03 (rev 223)** — the designer, against the built row: «delete "Pickup near Podil"». **It lasted exactly one revision** — it was a **new row at rev 220**, the second half of the line above, capitalised on the split — and it is struck rather than deleted so that neither the row nor the split is re-proposed later as new copy. **What was overruled is the reason rev 220 wrote down:** that note argued the shorthand restating the Route zone's **Pickup ▸ Podil, 21** one screen-height above was «the point rather than a defect», an offer card existing so the operator can decide without scrolling. She read the built card and decided the restatement was not worth the row. **The fact is not lost and gets more exact** — the Route zone reads *Podil, 21*, a street number against «near Podil». **`job-offer-inspection`'s parallel row went in the same pass** — **Obolon district**, which was the *identical* string to that file's Site ▸ `Location` value, so its case was the stronger of the two (restatement, **A8**) |
| job-offer | Offer | ~~· paid on completion~~ | Body | **removed 2026-09-03 (rev 220)** — the designer, pointing at the built string: «remove». Struck rather than deleted so the rewrite kept under it is not re-proposed later as new copy. **What it costs, stated:** the offer no longer says **when** the money lands, on the one screen the operator decides on in 8s. `operator-fee-terms` still says it — *Payment ▸ Auto-release ≤30 min after client confirms* — so the fact is in the product, one screen away and behind onboarding; `voice.md` **P3** (state it as a fact, before the tap) is what this spends, and it is hers to spend. **The pay itself does not move:** ₴180 keeps the `sr-only` **Pay** label welded to it inside its own wrapper, which is why that wrapper stays once its sibling goes. Removed from `job-offer-inspection` (₴650) in the same pass — one screen, one arrangement. Earlier: **changed in the Batch 2 rewrite** — `· pays on completion` → `· paid on completion`; the operator is the one paid, the job does not pay |
...
| listings | Search | Filters | Button | secondary; opens the Filters drawer. **Its name is *Filters* in every state from 2026-09-15** — the applied values are drawn as chips above it, and the designer took the count off the button (*"display applied filters in badges"*; her pick over keeping the count, both conforming). Until then an applied filter drew a count on the button, `aria-hidden`, and the name read *Filters, 3 applied* |
| listings | Search | Applied filters | Group name (`aria-label`, not drawn) | **new 2026-09-15** — names the chip row above *Filters* (`WCAG 1.3.1`). Present only while a filter is applied; the same words as `listings-filtered`'s row |
| listings | Search | Quadcopter · Hexacopter · FPV · Fixed-wing | Body (chip) | **new 2026-09-15** — DATA: an applied drone type, the drawer's own row label repeated on its chip, so the departure from `voice.md` P4 recorded on those rows travels with it |
| listings | Search | ₴200 – ₴600 | Body (chip) | **new 2026-09-15** — DATA: the applied price band, written the way the drawer writes its two ends (*₴0*, *₴1,000+*), joined by a spaced en dash — *₴0 – ₴600*, *₴200 – ₴1,000+*. A price stated as a fact (`voice.md` P3) |
| listings | Search | Remove filter: {value} | Button name (`aria-label`, not drawn) | **new 2026-09-15** — each chip's name, *Remove filter: Quadcopter*: the tap removes the filter, and the name says so. The pattern `listings-filtered` already speaks (*Remove filter: Today*), kept so one control has one name across the product (`WCAG 3.2.4`); the drawn value is inside the name (`WCAG 2.5.3`), and the drawn × is `aria-hidden` |
...
| listings | Filters drawer | ~~Which drone does the job~~ | Body | **REWRITTEN 2026-09-15 (rev 262)** — it stood one revision, from the preview she chose at rev 261, and was flagged there: *job* is the operator's word in `voice.md`'s vocabulary (client-side work is an *order*), and on a client surface the two are never crossed. Put to her with both readings, she took the rewrite. Struck rather than deleted, so it is not re-proposed |
| listings | Filters drawer | Which drone does your order | Body | the card's subtitle, `aria-describedby` on the group — **the designer's pick, 2026-09-15 (rev 262)**, of keeping *the job* as a recorded departure or this rewrite. Same shape, the client's own word for the work, addressed as *you* (`voice.md` Vocabulary, Form of address). WCAG and HIG pass either way |
...
| notifications | — | Notifications, 3 unread | Accessible name | new 2026-08-31 — the bell in `listings`' top bar. **An accessible name only**: the pill is `aria-hidden`, so the count would otherwise be a fact only a sighted reader has (`WCAG 4.1.2`). The comma, not a middle dot: this is read aloud, not laid out |
| notifications | — | Back to Home | Accessible name | new 2026-08-31 — **D9**: the chevron carries no visible words, and this name is invisible, so it neither competes with the title nor engages `WCAG 2.5.3` |
| notifications | Today | Today | Heading | **replaces *New*, 2026-09-03.** The log is grouped by the day a thing happened, not by whether it has been read. `HIG · Lists and tables` gives a recent date **relatively**, and *Today* is the word iOS itself uses in Notification Center and Messages — **P4**, the user's word, not the system's |
| notifications | Yesterday | Yesterday | Heading | **replaces *Earlier*, 2026-09-03.** The second and last relative step: anything older takes its own date, so *Yesterday* is the boundary between a word and a number. *Earlier* said only *not now*, and could not tell a two-day-old alert from a three-month-old one |
| notifications | 14 June | 14 June | Heading | new 2026-09-03 — **a format, not a fixed string**: `d MMMM`. Day-first, for UA and the EU; the month spelled out, because a heading is read once and *Jun* saved four characters nobody needed. The value is data; what is authored here is the shape |
| notifications | Today | New | Chip | new 2026-09-03 — the unread mark, moved **off** the group heading and **onto** the row. It had to move: once the groups are dates, an alert still unread from yesterday has no heading left to sit under. One word — still copy, never a colour — so `WCAG 1.4.1 Use of Color` stays un-engaged, exactly as it was when *New* was a heading |
| notifications | Today | Your delivery photo is ready | Row title | new 2026-08-31 — `CE-4`. **P1**, the outcome not the equipment; **O5**, *delivery photo* is the deliverable's one name, never *result* |
| notifications | Today | Package delivery · 10:12 | Row meta | **rewritten 2026-09-03** (was *Package delivery · Today, 10:12*) — the day left the row because the heading above it now carries the day, and saying it twice is the restatement **A8** exists to stop. Which order, then when. **P2**: the proof line, not decoration, which is why rev 209's optional `__desc` is spent here |
| notifications | Today | Ivan is 2 minutes away | Row title | new 2026-08-31 — `CE-4`'s own push, the *"Operator is 2 min away"* the sitemap has quoted since the first cut, said with the name the client has already been shown. **Operator**, never *pilot*; here his name, because by this point he is not a category |
| notifications | Today | Ivan K. accepted your order | Row title | new 2026-08-31 — `RJ-C1` / `RJ-C2`, the moment the void closes. **Order**, the client's word, never *job* |
| notifications | Yesterday | Rate your order | Row title | new 2026-08-31 — the one row that asks for something. Plain imperative, no *Please*, no exclamation. **Moved 28 Jun → Yesterday, 2026-09-03**, because the client's log held no yesterday and the pattern the designer asked for could not otherwise be seen on it |
| notifications | 14 June | Your photo and video set is ready | Row title | new 2026-08-31 — **O5**, the aerial deliverable's own name, matching `delivery-aerial`'s zone |
| notifications | 2 June | Your inspection report is ready | Row title | new 2026-08-31 — **O5**, the inspection deliverable's own name |
| notifications-empty | Notifications | No notifications yet | Title | new 2026-08-31 — the fact, not an apology. **Yet** is load-bearing: it says the screen is temporarily empty, not broken |
| notifications-empty | Notifications | Nothing has been sent to you yet. Start an order and its updates land here. | Body | new 2026-08-31 — `voice.md`'s empty rule: why it is empty, then the condition that ends it. Two sentences, the shape `listings-empty` set |
| notifications-empty | Notifications | Browse services | Button | 2026-08-31 — **not a new string**: `order-history-empty`'s own exit, reused. Same client, same door, and A8's rule (one phrasing per request) kept |
| notifications-loading | Notifications | Loading your notifications… | State message | new 2026-08-31 — `voice.md`'s loading rule: name what is loading. Built on `order-history-loading`'s *Loading your orders…*, one word apart, because it is the same act on the same list shape |
| operator-notifications | — | Notifications | Heading | new 2026-08-31 — the operator's screen title. The one string the two logs share, because the object is the same on both sides of the wall |
| operator-notifications | — | Notifications, 3 unread | Accessible name | new 2026-08-31 — the bell in `operator-listings`' top bar; identical to the client's, the count being a count |
| operator-notifications | — | Back to Jobs | Accessible name | new 2026-08-31 — **Jobs**, his home's name, where the client's says **Home**. `WCAG 4.1.2`, invisible, **D9** |
| operator-notifications | Today | ₴720 paid to your balance | Row title | new 2026-08-31 — `RJ-O3`. **paid**, the vocabulary's word for earnings landing, never *transferred* or *credited*; the amount leads because on this side the amount is the news |
| operator-notifications | Today | Job offer · Package delivery | Row title | **rewritten 2026-09-03** (was *New job offer · Package delivery*) — **the row's own chip already says *New***, so the title was restating it: the same duplication that took *Today,* out of the meta lines the same day. It is also what makes it fit — a title beside a chip is written to **260.72**, not 309, and at 17px the old string wrapped to two lines. Rephrased, never truncated. `OE-6`, the dispatch alert. **job**, never *order*: this is the operator's side of the wall. It is in the log although it is **not** in the settings card, and that is the point — `sitemap.md` bars silencing dispatch, so an offer that expired in a pocket must at least be findable |
| operator-notifications | Today | Podil → Osokorky · 10:45 | Row meta | **rewritten 2026-09-03** (was *Podil → Osokorky · Today, 10:45*) — the day left the row for the heading, as on the client's log. The route, then the time: the offer's own two facts, in the order `job-offer` states them |
| operator-notifications | Yesterday | Documents approved | Row title | **rewritten 2026-09-03** (was *Your documents are approved*) — it measured **250.77** against the **236.72** a chip-carrying title gets at the narrow tier (≤ 389px), so it wrapped on a 375px phone and nowhere else. **The pronoun is what was spent, and that is stated rather than smoothed:** *approved* was kept over *Your* because `operator-verification-loading` promises *We'll notify you when it's approved* and this row is that promise kept in the same word. Three titles on this screen already carry no pronoun, so the set is not broken. *Your documents cleared* fits too, at **203.33**, if the pronoun is worth more than the word-match. `MJ-2`'s gate clearing. Plain and factual; `operator-verification-loading` already promises *We'll notify you when it's approved*, and this is that promise kept, in the same words' register. **It is unread and it sits under *Yesterday*, and that is the whole reason the chip exists** — 2026-09-03: read-state and recency stopped being one axis the moment the headings became dates |
| operator-notifications | 28 June | Olena H. rated your job 5 stars | Row title | new 2026-08-31 — `EJ-3`. The client is named, which is what makes it standing rather than a statistic; **job**, not *order* |
| operator-notifications | 27 June | Withdrawal sent to your card | Row title | new 2026-08-31 — **Withdrawal**, the vocabulary's word for money going out, held apart from *paid* one group above. Two money events, two words, never crossed |
| operator-notifications | 28 June | 28 June | Heading | new 2026-09-03 — the same `d MMMM` format the client's log uses. **Today** and **Yesterday** are **not** re-inventoried for this screen: they are the identical two words on both sides of the wall — the vocabulary split governs *order* / *job*, not the calendar — and **A8** bars a second phrasing for the same fact. Only the shape is authored; the value is data |
| operator-notifications-empty | Notifications | No notifications yet | Title | new 2026-08-31 — the client's title, unchanged: the state is the same state and inventing a second phrasing would break A8 |
| operator-notifications-empty | Notifications | Nothing has been sent to you yet. Job offers, payouts and ratings land here. | Body | new 2026-08-31 — the second sentence changes and the first does not, which is the same reading rev 90 took on the two *Notify me* toasts: same act, different object. It names the three things that will arrive, in his words — job offers (`OE-6`), payouts (`RJ-O3`), ratings (`EJ-3`) |
| operator-notifications-empty | Notifications | Find jobs | Button | 2026-08-31 — **not a new string**: `wallet-empty`'s own exit, reused. Same operator, same door |
...
| operator-account-photo | Photo | JPG or PNG, up to 5 MB. Nothing changes until you save. | Body | P3 — the file rule and the fact that **no button on this screen commits on its own**, which is what makes `Delete` recoverable with no confirmation alert. **IT MOVED OUT OF THE ACTION BAR ON 2026-08-27 (rev 202)**, the designer on the built frame: «move this text to avatar background under the existing explenation» — an hour after the client screen took the same move at rev 201, which is the point: the twin stays a twin. It is now the second line of the photo card, under the sentence that names the reader, and the two ride `.dr-stack`'s 8 as one voice. `HIG · Action sheets`' message slot is **optional** and now empty; `WCAG 3.3.2` still has its instruction and now **earlier** in reading order; the three buttons keep `aria-describedby`, which computes by **id**, so nothing they announce changed (`4.1.2`, `1.3.1`). **TWO ROWS BY DECISION FROM 2026-08-27** — *«make in 2 rows but it should be more text in second row apply this chsnges for 2 profile roles»*, applied to this screen and the client's together: `.dr-note--balance` moves the break off the orphan `save.` and onto the sentence boundary, **`JPG or PNG, up to 5 MB.`** / **`Nothing changes until you save.`**. **No word changed** — the break is a property on the measure, never a `<br>` in the copy |
...
| ratings | Overall | ~~126 ratings · last 90 days~~ | Body | **removed 2026-09-03 (rev 219)** — the designer, pointing at the built line: «remove this frace». The score keeps no denominator: 4.8 out of 3 ratings and 4.8 out of 126 now read alike, and the per-service rows carry no counts either — `voice.md` **P2** (the proof, not the adjective) is what this spends, and it is hers to spend. Struck rather than deleted so the rewrite below is not re-proposed later as new copy. Earlier: **changed 2026-08-21 (rev 179)** — the leading `·` is gone. It had nothing on its left, and had not since the grayscale frame (`80b47c4`, 2026-07-02) wrote it as `.amount-sub` under a bare `4.8`: a middle dot joins two facts, and one hanging off the front of the first joins nothing. Her call: «забери спереду крапку». The two facts it does separate — the count and the window — keep theirs |
...
| result-upload-loading | Result photo | Sending the delivery photo. Keep the app open. | State message | `role="status"` **inside** the tray, under the upload glyph — the slot *Add photo* holds on the base frame (the designer, 2026-08-24: *«write this text under the loading upload icon»*). Re-homed from the takeover block that left 2026-08-21, so `WCAG 4.1.3` survives both moves; the `aria-hidden` came off the tray onto the glyph, since a live region inside an `aria-hidden` container is never announced. **The break is at the full stop, not where the box runs out (2026-08-24: *«речення Keep the app open make in the second row»*)** — a `<br>` after *photo.*, so line one says what is happening and line two says what you do about it. The line already ran to two (276.63 × 39.16 at 19.6 of leading) and still does, so no height moves; the string is unchanged and is still inventoried here once. `WCAG 1.4.10` / `1.4.12` are not engaged — a `<br>` ends a line without stopping the one before it from wrapping again at 200% text. `--slate` on `--media`, **5.51:1** at 14px |
...
| result-upload-loading | Action bar note | Payment is held until the client confirms — auto-confirms after 2h. | Body | **moved and re-faced 2026-08-24** — stood at the foot of `.dr-main` as `.dr-hint`, SF Mono 11px, left, after Notes. Two faults. **The face:** `.dr-hint` is the kit's loading voice and `kit.css:699` says why — *"mono, because it names structure that is still arriving"* — while this sentence is true before the upload, during it and after it. Its class is `.dr-note`, the kit's *"explanatory caption under a block"* (`kit.css:745`), which is what 40 pages spend on a standing condition, `result-upload-error` included, where the same term already reads *Payment stays held until the delivery photo uploads.* **The place:** the term is a condition of the submission, and `HIG` puts a term that takes effect on tap with the control that carries it — the shape `account-photo` already builds, `.dr-note` first in `.dr-actionbar` with the button `aria-describedby` it. That also closes `WCAG 1.3.1`: the sentence had no programmatic tie to anything. Contrast unchanged at **6.62:1** (`--slate` on `--page`, `WCAG 1.4.3` ✓); size 11 → 12, off HIG's 11pt floor; leading 1.4 → 1.5. String unchanged |
...
| switch-role | Confirm / Cancel | Switch to Operator | Button |  |
| switch-role | Confirm / Cancel | Cancel | Button |  |
...
| tracking · -empty · -error (+ the `-aerial` and `-inspection` trios) | Contact drawer | ~~Calls and messages go through DRON. Your number is not shared with the operator.~~ | Body | **REMOVED from all nine client drawers 2026-08-24**, in the same sweep as the operator's two — the designer's word on `job-checklist` (*«delete this sentence»*), the scope hers (*«everywhere»*). What it did: two facts, no adjective (`voice.md` **P2**), stated **before** the tap rather than discovered after it — **P3**'s discipline applied to privacy instead of to price. It answered the risk `ux-research/user-stories-cjm.md` logs 🔴 as *"direct channel bypasses platform"* by showing the client why the platform sits in the middle, and it was the reason the Call row could reach a masked line without the client wondering whose number it is. **What the removal costs, stated:** the mechanism is unchanged — the call still runs down DRON's own channel and no number appears on any frame — but **the client is no longer told why**, and that CJM risk is now answered by structure alone. The masking fact is now stated **nowhere in the product**, `call`'s own line having gone at rev 144. One instruction reverses it; the string is kept here for that reason |
