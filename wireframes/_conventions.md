...
- **Media / image / map** → a bordered `--wf-recessed` box with a centred uppercase **text label** of what it is: `MAP`, `OPERATOR PHOTO`, `RESULT PHOTO`, `INSPECTION IMAGE`. Keep the real aspect ratio (e.g. live map ≈ `343 × 200`). **Exception (2026-08-02):** where the designer has supplied the real asset, the label is replaced by the picture itself — the well then drops its border and padding and the image fills it (`.media-ph:has(img)` in `_wireframe.css`). Fifteen slots across nine pages carried a real picture the day the exception was written; counted again on **2026-08-16** the build stands at **37 pictures across 24 pages**, and every other well is still a label. `visuals/gaps.md` is the register of which is which.
  ...
  - **A filled upload tray (`.upload-zone:has(.up-shot)`, 2026-08-16).** The same exception reaches the operator's upload control, on the designer's instruction: *a user should see the result-proof image — the delivered package, the inspected roof — and the pictures for it are already in the repo.* An **empty** tray is the dashed `--wf-recessed` box with an uppercase label (`DELIVERY PHOTO`) and its `Add photo` button. **Filled**, it drops the border, the ground and the label — the label was standing in for exactly this picture — and the photograph covers a **180px** band. It is a photograph, so `cover`, never `contain` (§8's treatment split), and the band height is measured, not chosen: `delivered-at-door.jpg` is 652 × 522 into a 341px box, so `cover` draws it **341 × 273** and crops the middle. At **180** that cuts 47px top and bottom and the parcel survives; at the tray's own **140** it cuts 67px and takes the parcel — the proof the screen exists for — off the bottom edge. 180 is also exactly the well `delivery.html` draws, so the operator is looking at the frame the client will be shown. Filling the tray grows the page 40px, which is what an attachment does. **The control does not leave with the label:** `Replace photo` sits under the picture, because an attached photo has to be replaceable. `result-upload.html` is the only tray in this state; `operator-dispute`'s evidence tray stays a label because the picture it needs — an aerial frame that is genuinely soft — does not exist in the repo (`visuals/gaps.md` B3 #18).
