...
| Notifications — the alert log, OPERATOR (`OE-6` · `RJ-O3` · `EJ-3`) — **added 2026-08-31** (`_screens.md` O8). The mirror of the client's, reached from the **bell in `operator-listings`' top bar**, where it stands after the availability badge at the bar's trailing edge. Its rows name **jobs**, never orders (`voice.md` vocabulary), and open `job-offer`, `wallet`, `operator-verification`, `ratings`. | `operator-notifications.html` | `operator-notifications-empty.html` (nothing sent yet → **Find jobs**, the exit `wallet-empty` already uses). **No loading and no error page**, matching `wallet` / `ratings` |
