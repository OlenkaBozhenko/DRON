...
- **Where a two-line caption breaks is a decision, and it is made with a property, never with a `<br>`** (designer, 2026-08-27, on both photo screens: *«make in 2 rows but it should be more text in second row apply this chsnges for 2 profile roles»*). A sentence pair left to the default breaks wherever the measure runs out — on `account-photo` / `operator-account-photo` that put the one word `save.` alone on row two. `text-wrap: balance` (kit: `.dr-note--balance`) moves the break onto the **sentence boundary**, one fact per row: `JPG or PNG, up to 5 MB.` **139.05** / `Nothing changes until you save.` **180.14** in a 301 measure — the second row 8 characters and 41.09px longer, which is the shape she asked for. **It changes where rows break, never how many:** measured at 12px and again at a 2× text step, the paragraph is 2 rows / 36px and 3 rows / 108px **with the property and without it**, 0px of horizontal overrun either way. The string stays **one text node**, so `WCAG 1.3.1` / `4.1.2` announce the same sentence and a translation re-balances itself instead of inheriting an English break. **THE RULE TAKES ITS FIRST NAMED EXCEPTION, 2026-09-10, HERS** (*«make like in this picture»*, against two rendered options on the built `switch-role`). **The property could not reach the shape she set on 2026-08-27.** `text-wrap: balance` re-balances the rows a measure produces — it does not create them, and it only ever *evens* them: here it gave `Switching is always reversible` **167.61** / `from the same Account entry.` **166.86**, the first row longer, the opposite of *more text in second row*. A measure cannot reach it either: greedy wrapping was probed at 120 / 130 / 140 / 150 / 160 / 167px and **every** width that isolates `Switching is always` falls to **three** rows, because `reversible from the same Account entry.` will not sit in the same box. **So on this one line the break is markup**: `Switching is always <br>reversible from the same Account entry.` — **109.02 / 225.45**, the second row **116.43px and 19 characters longer**, which is the shape. **THE SPACE BEFORE THE `<br>` IS NOT OPTIONAL AND IT IS THE WHOLE ACCESSIBILITY COST OF THIS DEPARTURE:** a `<br>` is a line break and **not a word separator**, so `Switching is always<br>reversible` computes `textContent` **`alwaysreversible`** — one run-together word to a screen reader, to copy-paste and to any string extraction. With the space the accessible string is the sentence again, and the trailing space collapses at end of line: rows measure **109.02 / 225.45** with it and without it, **0px** of layout cost. **WHAT THE DEPARTURE ACTUALLY COSTS IS TRANSLATION:** the English break is frozen in the markup, so the Ukrainian string inherits a break chosen for English words and will need its own. **The property stays the default everywhere else** — both photo screens keep `.dr-note--balance`, and a `<br>` is reachable only where the shape she wants is provably out of the property's reach, measured and written down the way this one is. **AND THE EXCEPTION IS RETIRED THE SAME DAY IT WAS GRANTED — 2026-09-10 (rev 237), HERS:** *«Remove this and all the other content aligne by center of the page»*. The one line the `<br>` was written for is **deleted from `switch-role` along with its zone label**, so §95 stands again with **no live exception of its own**. **The grep that checked it found two other `<br>`s in product copy, and they are recorded rather than claimed away** — every `<br>` inside a `.wf-frame` across all 128 pages: `role-select`'s `.dr-lead` (*Pick a side to continue.<br>You can switch anytime…*) and `result-upload-loading`'s `.dr-status-line` (*Sending the delivery photo.<br>Keep the app open.*). Neither is a caption, so neither is inside this rule's literal scope, and neither was measured the way rev 236 measured this one. **Both carry the defect rev 236 caught here:** no space before the break, so `textContent` reads `photo.Keep` and `continue.You` — two words run together for a screen reader, for copy-paste and for any string extraction. Open for her word; not touched. The departure stays written down rather than erased: it is the record of what the property cannot do (create rows, or make the second one longer), so the same wall is recognised next time instead of being hit again. **What is still live from it is the accessibility half** — a `<br>` is a line break and never a word separator, so any future one keeps the space before it or `textContent` runs two words together.
...
- **Media / image / map** → a bordered `--wf-recessed` box with a centred uppercase **text label** of what it is: `MAP`, `OPERATOR PHOTO`, `RESULT PHOTO`, `INSPECTION IMAGE`. Keep the real aspect ratio (e.g. live map ≈ `343 × 200`). **Exception (2026-08-02):** where the designer has supplied the real asset, the label is replaced by the picture itself — the well then drops its border and padding and the image fills it (`.media-ph:has(img)` in `_wireframe.css`). Fifteen slots across nine pages carried a real picture the day the exception was written; counted again on **2026-08-16** the build stands at **37 pictures across 24 pages**, and every other well is still a label. `visuals/gaps.md` is the register of which is which.
  ...
  - **The upload tray, empty and filled (`.dr-upload` → `.dr-well--photo`, 2026-08-16; the base frame reverted **2026-08-21**).** The imagery exception reaches the operator's upload control, on the designer's instruction: *a user should see the result-proof image — the delivered package, the inspected roof — and the pictures for it are already in the repo.* **What it is not is the frame the operator arrives on**, and that is her correction of **2026-08-21**: *«тут має бути загрузити фото, кнопка "Replace photo" не активна відповідно»*. `flows.md` MJ-2 hands `Job checklist → Result upload` off **before** anything has been uploaded, so at the moment you land there is no photograph to draw; drawing one showed the screen's own gate — **proof before pay**, `H-7` — as already satisfied, and left the frame with no way to perform the one act it exists for. **The base frame draws the empty tray.** On the painted layer that is `.dr-upload[role="button"][tabindex="0"]` with `aria-labelledby` — the zone *is* the control, with no button inside it (`concept.md` rev 57 / 60) — at the kit's own floor `--sz-upload-min` **140**, `--media` `#E4E1DA` at `--r-media` **13**, the 32px upload glyph at `--slate` **5.51:1**, and the verb **`Add photo`** in `.dr-upload__action` (15 / 600) at `--ink` **13.33:1**. It takes the **well** form and not `--card`, because the box it turns into when it fills is `--media` at `--r-media` as well: empty and filled are **one box**, not two components. On the grayscale layer the same tray is still the dashed `--wf-recessed` box with the uppercase `DELIVERY PHOTO` label. **`Replace photo` does not leave with the picture — it goes inactive.** `.dr-btn:disabled` is `--slate` on `--btn2` `#D6D2C9`, **4.77:1**, built and never dimmed (`concept.md` §0); the control keeps its place, so nothing under the thumb moves when the photo lands — only its state changes. **Filled**, the tray drops the glyph and the verb — they were standing in for exactly this picture — and the photograph covers a **180px** band. It is a photograph, so `cover`, never `contain` (§8's treatment split), and the band height is measured, not chosen: `delivered-at-door.jpg` is 652 × 522 into a 341px box, so `cover` draws it **341 × 273** and crops the middle. At **180** that cuts 47px top and bottom and the parcel survives; at the tray's own **140** it cuts 67px and takes the parcel — the proof the screen exists for — off the bottom edge. 180 is also exactly the well `delivery.html` draws, so the operator is looking at the frame the client will be shown. Filling the tray grows the page 40px, which is what an attachment does. `result-upload.html` is the only tray with a filled state; `operator-dispute`'s evidence tray stays a label because the picture it needs — an aerial frame that is genuinely soft — does not exist in the repo (`visuals/gaps.md` B3 #18).
...
| A caption line (`.dr-note`) | `order-setup-error` — **one screen, from 2026-08-27** | states a fact about the action, not about `main`. **BOTH PHOTO SCREENS LEFT THIS LIST ON 2026-08-27**, `account-photo` at rev 201 and `operator-account-photo` at rev 202 an hour behind it: the designer moved the file rule up into the photo card on each, so both bars are buttons and nothing else. The three buttons on each keep `aria-describedby="photo-rules"` pointing into `main`, because the description resolves by **id** and not by proximity — `WCAG 4.1.2` / `1.3.1` unmoved, `3.3.2` met earlier in reading order. `HIG · Action sheets` calls that message slot **optional**, so an empty one is a permitted shape and neither screen owes a departure for it. The rung stays declared and spent once |
...
|---|---|---|
| Home / start an order — **a search from 2026-09-14** (`_screens.md` §3): three service badges, Date · Time, Filters · Find. The Date, Time and Filters drawers are controls on the page, not files. | `listings.html` | — *the pages below predate the search and still draw the service list; open, `_screens.md` §3* — |
| Results on the map — **new 2026-09-14** (`_screens.md` §3a). What *Find* opens: the live map and a docked drawer of choices — picture, outcome, available, arrives in, price — and *Order*. **One file per service**, the rule `order-setup` set: the rows are that service's. | `listings-map.html` | `listings-map-aerial.html` · `listings-map-inspection.html` — base only |
| Home / start an order **+ Service catalogue** (merged per `sitemap.md §7.3`) — *the service-list pages* | *(was `listings.html`)* | `listings-filters.html` (filter panel open — Location / Time / Price), `listings-filter-country.html` · `listings-filter-region.html` · `listings-filter-city.html` · `listings-filter-district.html` (the place pick, four dependent levels pushed from the Location rows), `listings-filtered.html` (filters applied — the All chip + removable chips), `listings-empty.html` (no results / loosen filters), `listings-error.html` (load failure), `listings-loading.html` |
| Order setup / details — **one file per service, 2026-08-16** (`_screens.md` §5). The nav title names the service, and the form asks what that service needs: **delivery** Pickup · Drop-off · Parcel size → `order-review`; **aerial** Location · Shoot · Duration · What you get → `time-slot`; **inspection** Location · Structure → `time-slot`. The two calendar-first services (`sitemap.md` CE-2) are what give `time-slot` its entrance. | `order-setup.html` | `order-setup-aerial.html` · `order-setup-inspection.html` — the other two services, base only. **The two state pages stay on the delivery file and are not repeated**: *none available now* is a fleet fact and *address outside service zone* is a coverage fact — neither changes with the service. Also `order-setup-empty.html`, `order-setup-error.html` |
| Order review & price — **one file per service, 2026-08-16** (`_screens.md` §6), following the setup split. The summary states what *that* service was asked for and the breakdown prices *that* service's work: **delivery** Base fare · Distance · Service fee = ₴180; **aerial** Shoot · 30 min · Photo &amp; video · Service fee = ₴800; **inspection** Inspection · 3 parts · Inspection report · Service fee = ₴650. The deliverable takes its own price line, so a client choice that moves the total shows what it costs (`voice.md` **P3**); every breakdown reconciles exactly. The two calendar-first files are reached from `time-slot`, so their back control names *Pick a time*. | `order-review.html` | `order-review-aerial.html` · `order-review-inspection.html` — base only. **`order-review-loading.html` stays on the delivery file and is not repeated** — open, not decided: its skeleton is the price, but the resolved summary above it is delivery's. Also `order-review-loading.html` |
| Payment *(base = card entry; the method is chosen in the Pay drawer on `order-review`)* — **one file per service, 2026-08-16**: the amount and its subtitle are the only things that move (₴180 delivery · **₴800** aerial · **₴650** inspection). | `payment.html` | `payment-aerial.html` · `payment-inspection.html`, `payment-error.html`, `payment-loading.html` — **the two states are service-neutral and are not repeated** |
| Order confirmed / operator revealed *(base = success)* — **one file per service, 2026-08-16**. The two calendar-first files **state the booking instead of asserting motion**: *Your shoot / inspection is booked for Wed 2 Jul, 09:00*, rows **When** and **Booked**, not an ETA and not *En route to pickup*. The operator card and both trust badges are unchanged — `RJ-C1` does not weaken because the operator comes on Wednesday. | `order-confirmed.html` | `order-confirmed-aerial.html` · `order-confirmed-inspection.html`, `order-confirmed-empty.html`, `order-confirmed-error.html`, `order-confirmed-loading.html` |
| Live tracking & ETA *(the **Contact operator** CTA opens a drawer — Call operator · Chat with the operator · Contact support — on all three states that carry it; `tracking-loading` has no operator card and no button)* — **one file per service, 2026-08-16**: `tracking-aerial` · `-aerial-empty` · `-aerial-error` say *En route to the shoot* and hand **View details** to `delivery-aerial`; `tracking-inspection` · `-inspection-empty` · `-inspection-error` say *En route to the site* and hand it to `inspection-report`. Map well, four-node rail, operator layer and the drawer are byte-identical across all of them; `tracking-loading` is service-neutral and is not repeated. | `tracking.html` | `tracking-aerial.html` · `tracking-aerial-empty.html` · `tracking-aerial-error.html` · `tracking-inspection.html` · `tracking-inspection-empty.html` · `tracking-inspection-error.html`, `call.html` · `chat.html` (sub-views — the two channels the drawer opens, both base only; **both are DRON's own screens, neither hands off to the OS**, because the operator's number is masked. `call.html` is the product's **one dark screen**, built to the designer's Figma reference `99:26` — the platform's convention for a call UI, not a second theme; its portrait disc holds a placeholder glyph, `visuals/gaps.md` B1 #4, the one human cutout being shot from behind). Also `tracking-empty.html`, `tracking-error.html`, `tracking-loading.html` |
| Delivery confirmation *(base = success)* — **the aerial variant was added 2026-08-16**; inspection needs none, its deliverable screen being `inspection-report.html`. | `delivery.html` | `delivery-aerial.html` — topbar *Done*, zone *Photo &amp; video set* (`voice.md` **O5**, named for what that order asked for), the set's photograph in the well and the video on the record. Nothing on the screen says *delivery*; the file name is a file name. Also `delivery-error.html`, `delivery-loading.html`, both service-neutral |
...
| Incoming job offer / accept-decline — **one file per service, two of them, 2026-08-21** (`_screens.md` §13). The offer card, the second zone and the countdown are the ones `operator-listings` writes on that card, so the screen the tap opens is the job the tap named (`WCAG 2.4.4`, level A). A delivery reads **Route** — pickup → drop-off; an inspection reads **Site** — location · structure. **The inspection offer carries the client’s note from 2026-08-24** (her call, reversing the 2026-08-21 reading that kept it off; `concept.md` rev 195): the same `Notes ▸ client instructions` card the brief carries, the same string, sitting under Site and above the countdown — measured at **577 of 577**, no scroll, the countdown ending 125.83px clear. The delivery offer does not carry its note yet; that is an open question on `_screens.md` §13, not a decision. No `-aerial` file — that job is already accepted, so it has no offer. | `job-offer.html` | `job-offer-empty.html`, `job-offer-inspection.html` |
...
| Time-slot pick — calendar-first (`C-2`). **Split per service 2026-08-16**, two files beyond the thirteen the review-chain pass authorised: one shared file could point its five exits at only one review, which left six of those thirteen unreachable by walking the prototype. `time-slot.html` **is the aerial instance** — renaming it `time-slot-aerial.html` is the designer's call, not assumed. | `time-slot.html` | `time-slot-empty.html`, `time-slot-inspection.html`, `time-slot-inspection-empty.html` |
...
| Notifications — the alert log, CLIENT (`RJ-C2` · `CE-4`) — **added 2026-08-31** (`_screens.md` C10). Reached from the **bell in `listings`' top bar**, not from a tab. A pushed screen: back chevron + the title *Notifications*, and **no tab bar** — the shape `order-details` set. The list is grouped **by day** with `.dr-sect` — **Today**, **Yesterday**, then `d MMMM` (**14 June**, **2 June**) — spending a rung that had been declared and applied on 0 of 80 pages. **It was grouped New / Earlier until 2026-09-03**, when the designer asked for the relative dates `HIG · Lists and tables` prescribes; the unread signal those two words carried moved onto the row as a **New** chip, so `WCAG 1.4.1` is still not engaged. The groups sit in a `.dr-log` column whose gap is `--sp-list` **28px** against the rows' **8px** — a **3.5:1** break, her call, taken against 20 and 24. Every row is an `<a>` into the screen its alert names — `tracking`, `delivery`, `inspection-report`, `rate`, `order-details-aerial`. | `notifications.html` | `notifications-empty.html` (nothing sent yet → **Browse services**), `notifications-loading.html` (fetching — the skeleton is three `.dr-choice`-shaped rows). **No error page**, matching `order-history`; traced in `flows.md` and recorded as a decision |
| Notifications — the alert log, OPERATOR (`OE-6` · `RJ-O3` · `EJ-3`) — **added 2026-08-31** (`_screens.md` O8). The mirror of the client's, reached from the **bell in `operator-listings`' top bar**, where it stands after the availability badge at the bar's trailing edge — the designer's own choice 2026-09-01, both orders having been drawn and measured for her. Its rows name **jobs**, never orders (`voice.md` vocabulary), and open `job-offer`, `wallet`, `operator-verification`, `ratings`. Grouped by day like the client's since 2026-09-03 — **Today**, **Yesterday**, **28 June**, **27 June** — and it is this log that shows why read-state left the heading: *Your documents are approved* is **unread under Yesterday**, which the old **New** / **Earlier** pair could not say. | `operator-notifications.html` | `operator-notifications-empty.html` (nothing sent yet → **Find jobs**, the exit `wallet-empty` already uses). **No loading and no error page**, matching `wallet` / `ratings` |
