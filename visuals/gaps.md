...
| ~~3~~ | ~~`people/person-client-olena.png`~~ | **Closed 2026-08-02.** The designer supplied Figma node `75:157` — her own photograph, outdoors, ordinary clothes, no lime and no uniform, which is what this row asked for. Unlike Ivan's it is a **photograph with a background**, not a cutout, so the circle needs no fill of its own. **Recropped the same day:** as first placed it framed her and the dog, and at 56px her face held about a third of the circle while at 28px it went to mush. Retightened to **52% of the frame width, centred on the face** — the same treatment Ivan's took, so two avatars in one product crop alike. The wider frame is still in the master if it is wanted back. | `account` **(filled)** | — |
| 4 | `people/person-operator-andriy.png` | Andriy M. face forward — the same operator `tracking` already shows from behind. Needed because a round avatar cannot crop a back-of-head | `order-confirmed` (56px), **`call`** (added 2026-08-16 — **176px, the largest portrait slot in the product**, and currently the kit's outline person glyph on `--night-raised`; the designer's own reference shows a placeholder here too) | §5.2 operator, same pose |
...
| ~~17~~ | ~~`scenes/aerial-trukhaniv-island.jpg`~~ → `scenes/aerial-rafting-run.jpg` | **Filled 2026-08-16 — with a different subject than this row asked for, and the row is corrected to the picture rather than the other way round.** The designer supplied Figma node `96:14` (file `YlGWlsWWjKSCxhONMzGG2F`), 949 × 638: a raft of paddlers — six in green and yellow plus a guide in black at the stern — dropping through white water between wet rock walls, shot from above. Not the riverside plot at 60 m this row imagined — an **event** shoot, which is the first use the `listings` card names (*Events, real-estate listings, personal occasions*). Saved by what it shows, the way #12 and #13 were: `aerial-rafting-run.jpg`, master 949 × 638 + `thumbs/` at 652 × 438. Measured against `README.md` §3: **warm skew +5.9, in the +4…+18 band**; **ground hue 82.5°, outside the 30–45° band** — the saturated pixels are gorge moss and foliage, so the picture reads green rather than warm. Recorded, not corrected (§7.2) — it is her frame, and the neutrals are in band. **Where it landed:** `order-details-aerial`, the new record file, **not** the `order-history` card — the cards keep their drones by her call of the same day (*«верни назад дрони, а коли я відкриваю картку щоб подивитись деталі»*), which is the whole reason those record files exist. **What this row got wrong, kept visible:** ~~its two neighbours show what came back and it still shows an airframe~~ — the reading that survived the same-day reversal is the opposite one, and the card was never the slot. |
| ~~19~~ | ~~`scenes/aerial-video-cover.jpg`~~ → `scenes/aerial-rafting-boulders.jpg` | **Opened and filled in the same hour, 2026-08-16.** The aerial record's zone is named *Photo & video set* and held one photograph, so the video it promised had no picture at all — and the first build had to use the photo card's own frame as the poster. The designer supplied Figma node `97:22` (file `YlGWlsWWjKSCxhONMzGG2F`), 599 × 362: a grey raft with a crew in red running a rapid between dark boulders, shot from above — a **different** run from `aerial-rafting-run.jpg`, which is what the row needed: two files in one set are told apart by what they show. Saved by what it shows, not by where it sits, like #12. Master and `thumbs/` copy are the same 599 × 362 pixels — the CSS box is 341 × 180, so the supplied file is already ~1.76× it and there is nothing to reduce. | `order-details-aerial` — the video card's preview |
...
| ~~20~~ | ~~`scenes/aerial-rafting-crew.jpg`~~ | **Opened and filled in the same pass, 2026-08-16 (concept.md rev 153) — and registered late, because the file was already in the build.** The aerial delivery screen returns a *set*, not one file, and the pile that shows it had been carrying three photographs with no row here at all. The crew head-on in white water, helmets up and both blades in the air. **1514 x 1009 master, 432 x 288 thumb** (2.09x the CSS box). Checker: warm skew **+9.7 [in band]**, ground hue **29.9** against the 30-45 target, no payload lime — correct for a client's own footage. Hue 0.1 outside the band on a river gorge is **recorded, not corrected** (README section 7.2). | `delivery-aerial` — photo pile, 1 of 3 **(filled)** |
| ~~21~~ | ~~`scenes/aerial-rafting-drop.jpg`~~ | **Same pass.** The raft airborne off the waterfall, the whole crew's arms raised — the frame that makes the set worth having, and the reason the deliverable is a pile rather than a single picture. **1488 x 992 master, 432 x 288 thumb.** Checker: warm skew **+7.3 [in band]**, ground hue **29.0**, no payload lime. Recorded, not corrected. | `delivery-aerial` — photo pile, 2 of 3 **(filled)** |
| ~~22~~ | ~~`scenes/aerial-rafting-overhead.jpg`~~ | **Same pass.** The raft from directly overhead, running the rapid between wet rock and green water — the drone-native angle, and the one frame that could only have been shot from the air. **1516 x 1011 master, 432 x 288 thumb.** Checker: warm skew **-8.7 [OUT]**, ground hue **177.1** — teal, because a rapid seen from directly above is green water and nothing else. **The coolest frame in the scene library**, cooler than `-boulders` at -10.1 / 191.3. **Recorded, not corrected** (README section 7.2): it is user content, and a client's own footage is not graded to the page. | `delivery-aerial` — photo pile, 3 of 3 **(filled)** |
...
| `order-history` — past cards | ~~The two settled cards could take `delivered-at-door` and `inspection-roof-cracked-tiles` today.~~ **Done the same day — she asked for it, and gave the reason the audit did not have:** *a client who has had the same roof inspected three times sees three identical airframes and cannot tell the orders apart.* The 28 Jun card took `delivered-at-door` and the 2 Jun card `inspection-roof-cracked-tiles` — **and she reversed it the same day**, which is the half this row did not record until 2026-08-16 (rev 153): *«та не на ці картки фото, верни назад дрони, а коли я відкриваю картку щоб подивитись деталі»* — not photographs on these cards; put the drones back, and put the picture where the card is opened. **Verified against the build: all four wells carry drone renders** (`drone-08` twice, `drone-flight-camera`, `drone-flight-clean`), and the photographs live on the `order-details` records the cards open, which is where the per-service split put them. B3 #17 has since been filled (rev 135) and the library now holds five aerial frames — #17 · #19 · #20 · #21 · #22 — so the aerial card keeps its drone by the reversal, not for want of a picture. | **reversed · drones stand** |
