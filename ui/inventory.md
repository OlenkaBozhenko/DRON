...
| **Language switch** | `.wf-lang` (shell-injected) | 40 · 3 painted | **painted 2026-08-02** | EN / UA in every top bar, injected by `_wf-shell.js`, so it is the one control that stands on **all 40 screens**. Painted from the kit's **tab** values, not the primary button's: a solid `--green` pill would be a second green control on a screen that already spends its one on the CTA, and `DESIGN.md` caps that at one per screen at ~5% of the frame. Selected `--green-wash` + `--ink` — **14.75:1**; unselected transparent + `--slate` — **6.62:1**; hover `--card` + `--ink` — **14.37:1**; focus `2.5px --ink` — **15.99:1**; track `--line`, `--r-btn` 12. Buttons are **44 × 44**, so `HIG · 44pt` passes. The fill step is **1.085:1** against the page and the track border **1.34:1** — both below the 3:1 `WCAG 1.4.11` asks of a state indicator on its own, which is exactly why the ink moves `--slate` → `--ink` with it: **two signals where the convention gives one**, the same reading the tab bar took. `aria-pressed` carries it for assistive tech. **Scoped to `.dr`** — the other 30 screens keep the grayscale set's inverted-black pill, which is what every chip and group tab there already does. |
