...
| Component | Classes | Screens | Status | What to decide |
|---|---|---|---|---|
| **Price line** | `.price-line`, `.lbl`, `.val` | 5 · 0 | grayscale | The painted fee row solved this as a caption over a tabular figure — **and the caption was removed 2026-08-02 at the designer's call**, so what ports is a bare `number` step baseline-aligned against a chip. The words survive as an `.sr-only` accessible name, because a figure with no label has no programmatic relationship (`WCAG 1.3.1`). **Decide before porting:** these five screens run a *labelled* price line (`Subtotal`, `Total`), where the label distinguishes two figures on one screen — the order card had only one figure to name. |
