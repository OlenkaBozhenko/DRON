...
| **Language switch** | `.dr-lang` · `.wf-lang` (shell-injected) | 40 · 40 | **painted 2026-08-02, all 40** | EN / UA in every top bar, so it is the one control that stands on **every screen in the product**. **Painted from `.dr-segment`, not from the tab and not from the button** — it *is* a segmented control, so it takes that structure: a recessed `--media` track with a `--page` plate raised inside it, no border, no dividers, the plate carrying the state. That also keeps green out of it, which matters here more than anywhere: this control is on all 40 screens, and a green pill would be a second green control on **each** screen that already spends its single accent on the CTA. **Measured, painted / grayscale:** selected label on the plate **15.99:1 / 15.33:1**; unselected on the track **5.51:1 / 5.32:1**; plate against the track **1.20:1 / 1.30:1**. That last pair is under the 3:1 `WCAG 1.4.11` asks of a state indicator standing alone — which is why the ink moves `--slate` → `--ink` with the plate: two signals where the convention gives one, and `aria-pressed` carries it for assistive tech. **Geometry, 2026-08-02:** was 90 × 46, now **72 × 32** on the designer's instruction. It shares a 56px bar with a title, and the full 44pt control crowded it. The target comes back vertically through a transparent `::after{inset:-8px 0}` — the button box is 28, so −8 top and bottom is **exactly 44**, the same trick iOS uses on its own 32pt controls and the one the in-page tabs took above. Horizontally each half stays its own width: iOS's segmented control is 32pt tall with segments as wide as the label needs, and widening each to 44 would put this back at 92px. **One rule for all 84 files.** The kit is the source — `ui/kit.css` `.dr-lang` — and the shell writes each value as `var(--token, fallback)`, so the 10 painted frames resolve the token and the other 74 fall through to the `_conventions.md` §4.1 grayscale ramp. One component, one set of decisions, two palettes. |
