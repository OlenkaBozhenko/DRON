...
| **Search row** | `.search-row`, `.field` | 2 · 0 | grayscale · **settled** | The one field that keeps a ground, because it is not in a grouped list — a filled rounded rect on the page, `--media` at **`--r-input` 10px**, no border. That is the iOS search bar, and it finally gives `--r-input` its only home: declared since §4 and drawn nowhere until now. Value `--ink` **13.33:1**, placeholder `--slate` **5.51:1**. |
...
| 2b | **One card or two?** | The client listing card and the operator job card were built separately and never reconciled. |
