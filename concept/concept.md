...
Studio is developed into a full design-language showcase (the `.studio-lab` block in `directions.html`, and embedded in the artifact's §06), driven from the designer's Figma reference board (`node 833-49948`). Per the designer, the reference's **electric lime `#C4F000`** is now the Studio primary (charcoal on top, ~13:1), replacing `#9bcf4a` for this direction, and **surfaces are soft grey, not pure white**.
...
- **Palette (grey + lime)** — Mist `#E7E9ED` page · Surface `#F3F4F6` card · Media `#DBDEE3` (drone / map / imagery wells) · Ink `#1A1A1A` · Slate `#565A62` · Hairline `#D3D6DC` · Signal Lime `#C4F000` (primary / live) · Graphite `#1F2124` (dark secondary / dark surface). Verified AA: ink 15.8:1 on card, slate 6.3:1 / 5.1:1, charcoal-on-lime 13.1:1, dark-lime pill text 6.9:1. Lime is fill-only, never text.
...
- **Buttons & actions** (Figma-inspired) — **solid lime** primary (charcoal on top), **no gradient**; **graphite** secondary (`#1F2124`, white); and card **secondary circular actions are graphite & hairline** (lime reserved for the primary action + status). Full state set (hover / active / focus-visible / disabled).
- **Card variants** — live order + route timeline (Accepted → En route → On-site → Done) with ETA; operator / fleet-unit card with a **drone illustration** banner (certified + rating + call / navigate); parcel card with a weight / dimensions / insured stat trio; delivery-status row with a chevron. Cards laid out 2-up so the header never wraps.
- **Map & route** — line-grid grey map with a lime traveled path + dashed remaining, a **drone (quadcopter) marker** mid-route, a destination pin, an "Arriving in" ETA chip, and a live-tracking footer.
- **Imagery** — a **drone (quadcopter) illustration** on **light** (charcoal on grey) and **dark** (lime glows over graphite) surfaces. Vector SVG line-art for now (CSP-safe); a real generated/photographic drone image can be embedded as a base64 data-URI when provided.
- **Backgrounds & textures** — Mist, Surface, Green wash (flat tint), Graphite, Dot grid, Line grid, Hatch, Contour. Subtle surface treatments only; no colour gradients (the earlier green-glow radial was dropped).

Verified in-browser (desktop + mobile): no horizontal overflow, outline icons only, solid-fill buttons only, AA contrast held. **Applied fully in the artifact (§06); `directions.html` studio-lab currently has the lime + grey but not yet the outline-icon / drone / rounded-palette parity.**
