...
- **2026-08-01 (rev 18)** — Designer note on `order-history`: the `.stat3` inset strip is now a **live-state** surface, not a card default. A delivered order's From▸To row drops the tile and sits flush at the card's own inset, so header, route and fee share one left edge; separation comes from the card's 20px group gap, the same interval that already divides every other group. Rationale: on a finished order the route is a record, not a fact still in motion, and the tile was the heaviest mark on a card whose only remaining job is to be scanned. The live card keeps the strip. Text moves from the blend onto `--card` — `#1A1A1A` 15.2:1 and `#5A5751` 5.95:1 on `#ECE9E4`, both already documented. Delivered card height 250 → 218.
