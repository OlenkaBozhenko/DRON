...
| `drone-06.png` | White | twin lenses | lime cargo box | ¾ side, angled | Activity-list thumbnails — *superseded on `order-history`*: every fleet unit carries the same lime box, so a list mixing delivery, photo and inspection reads as three deliveries. That surface uses the §6.2 cutouts, where the payload **is** the service |
...
| `drones/drone-flight-delivery.png` | In flight, cargo box on a tether | 869 × 477 | **Package delivery** service card · live tracking · order-history card well |
| `drones/drone-flight-camera.png` | In flight, large gimbal camera | 392 × 291 | **Aerial photo / video** service card · order-history card well |
| `drones/drone-flight-clean.png` | In flight, no payload, amber blade tips | 409 × 174 | **Inspection** service card · neutral use · order-history card well |
